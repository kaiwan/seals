...
This file is, by default, named `build.config` and is a symolic (or soft) link to the `build.config.arm32_vexpress` build config file - to build the ARM-32 Verstaile Express!
...
To go ahead with the current selection, simply press `Esc` here...
...
This allows you to review it and decide if it's fine; if your answer is:<br>
...
5. When happy with the state of the build config, run the script:
